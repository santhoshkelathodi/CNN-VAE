...
#newcode
train_path = 'T15/'
test_path = 'T15/' # as of now giving both training and test as same data. But need to segregate later
checkpoint_dir = "T15/models/"
results_dir = "T15/results/"
result_prefix = "t15_"
img_size_height = 120
img_size_width = 120
...
#newcode
batch_size_images = 200 # 200 good #20 originally tested 600 dont test. slows down your machine      # Batch size
...
with tf.Session(graph=graph_images) as sess_images:
    ...
    for e in range(epochs_images):
        ...
        for x, y in get_batches(X_tr_images, y_tr_images, batch_size_images):
            ...
            # Iterate
            images_iteration += 1
        saveName =  checkpoint_dir+"15_class_overall.ckpt"
        #print saveName
...
with tf.Session(graph=graph_images) as sess_images:
    ...
    plotMatrix(cm, data_labels, "T15", results_dir+result_prefix+'grey_confusion_mat.png')
    #end:new code
