...
#newcode
train_path = 'QMUL/'
test_path = 'QMUL/' # as of now giving both training and test as same data. But need to segregate later
checkpoint_dir = "QMUL/models/"
results_dir = "QMUL/results/"
result_prefix = "qmul_"
img_size_height = 120
img_size_width = 120
...
classes_images = ['0', '1', '2', '3', '7_9_10', '8', '12']
...
fig.savefig(results_dir+result_prefix+'loss_plot.png')
...
plt.legend(['train', 'validation'], loc='upper right')
#plt.show()
fig.savefig(results_dir+result_prefix+'accuracy_plot.png')
#newcode
#'''
...
def plot_confusion_matrix(cls_pred,cls_true, num_classes):
    ...
    #plt.show()
    plt.savefig(results_dir+result_prefix+'confusion_mat.png', cmap='grey')
...
with tf.Session(graph=graph_images) as sess_images:
    ...
    result, input, result_accuracy = sess_images.run([pred_class,input_class, images_accuracy], feed_dict=feed)
    print("Test 1 accuracy: {:.6f}".format(np.mean(result_accuracy)))
    #print res_pred1[0]
    #print result_pred
    ...
    cm = cm*100
    print(cm)
    data_labels = ['1', '2', '3', '4', '5', '6', '7']
    plotMatrix(cm, data_labels, "QMUL", results_dir+result_prefix+'grey_confusion_mat.png')
    #end:new code
